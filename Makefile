...
debug.gateway:
	go run ./cmd/kube-agent-gateway --jwks-endpoint=https://srv-octohelm---infrav2.hw-dev.rktl.xyz/octohelm/.well-known/jwks.json --port=8080

debug.agent:
	go run ./cmd/kube-agent --gateway-address=127.0.0.1:8080 --host=hw-dev --bearer-token=$(shell cat ./deploy/clusters/agent-token.txt)
