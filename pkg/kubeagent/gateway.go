...
func (g *Gateway) agentsHandler(rw http.ResponseWriter, req *http.Request) {
	...
	g.channels.Range(func(key, value interface{}) bool {
		channel := value.(*Channel)
		agentCounts[channel.ChannelMeta.AgentHost]++
		return true
	})

	memberAgentsChannels := map[string]map[string]int{
		g.Addr(): agentCounts,
	}
	...
	if req.URL.Query().Get("single") == "" {
		...
		chMemberAgentsChannels := make(chan map[string]map[string]int)
		defer func() {
			close(chMemberAgentsChannels)
		}()

		go func() {
			for c := range chMemberAgentsChannels {
				for m, s := range c {
					memberAgentsChannels[m] = s
				}
			}
		}()
		...
		for i := range hosts {
			...
			go func(host string) {
				...
				if c != nil {
					...
					if resp != nil {
						memberAgentsChannels := map[string]map[string]int{}
						_ = json.NewDecoder(resp.Body).Decode(&memberAgentsChannels)
						_ = resp.Body.Close()
						chMemberAgentsChannels <- memberAgentsChannels
					}
				}
			}(hosts[i])
		}
	...
	rw.WriteHeader(http.StatusOK)
	rw.Header().Set("Content-Type", "application/json;charset=utf-8")
	_ = json.NewEncoder(rw).Encode(memberAgentsChannels)
}
